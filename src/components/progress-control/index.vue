 ...
 * @LastEditTime: 2024-06-01 23:11:32
 * @Description: 单一文件上传
-->
<template>
  <div class="container">
    <div class="item">
      ...
      <section class="upload_box" ref="upload1">
        ...
        <input type="file" ref="avatFile" @change="change" class="upload_inp" accept=".pdf, .png, .jpg, .jpeg" />
        ...
        <div class="prg">
          <div ref="pgBox" :style="{ width: `${pgWidth}` }" class="box"></div>
        </div>
      </section>
    </div>
  </div>
...
const uploadServer = async () => {
  ...
  try {
    ...
    let resData = await api.post('/upload_single', formData, {
      ...
      onUploadProgress: (e) => {
        let { loaded, total } = e
        pgWidth.value = `${(loaded / total) * 300}px`
        console.log(pgWidth.value, loaded, total)
      },
    })
    if (resData.code === 0) {
      pgWidth.value = '100%'
      await delay(200)
      alert('文件已经上传成功！', `您可以基于 ${resData.servicePath} 访问这个资源`)
      return
    }
