 ...
 * @LastEditTime: 2024-06-02 09:11:49
 * @Description: 单一文件上传
-->
<template>
  <div class="container">
...
const uploadServer = () => {
  ...
  let fObj = {
    // 为了传输数据的过程中，数据不会丢失或者被误解，所以需要URI 转码， 防止传输的过程中乱码
    file: encodeURIComponent(fileBase64.value),
    filename: fileName.value,
  }

  // URL编码字符串可以和下面请求头一起工作
  let headers = {
    headers: {
  ...
  api
    .post('/upload_base64', fObj, headers)
    .then((res) => {
      if (res.code === 0) {
...
const change = async (event) => {
  ...
  //转based64
  Base64 = await changeBase64(fileData)
  fileBase64.value = Base64

  console.log(fileBase64.value, fileName.value)
  showList = true
  showTips = false
  console.log(target)
}
